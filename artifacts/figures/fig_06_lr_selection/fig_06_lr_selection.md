...
Validation NLL on the small-slice learning-rate sweep used to select peak LR before the main training runs (512 train rows, 128 validation rows, seed 7). Both attention-only LoRA (blue, circles) and all-layer LoRA (orange, squares) minimize at 3e-4, highlighted by open markers. The valley shapes differ sharply: attention-only NLL is nearly flat after the minimum (0.3633 at 3e-4, 0.3644 at 1e-3, Δ +0.001), while all-layer rises steeply (0.3560 at 3e-4, 0.3740 at 1e-3, Δ +0.018), indicating greater LR sensitivity under the higher-capacity adapter.
