#!/usr/bin/env python3
"""
Build the strict original-only OpenMath raw dataset from the `gsm8k` and `math` sources in
OpenMathInstruct-2 `train_1M`, using the shared quality gate from the curation scripts.

Usage:
  uv run --with datasets python scripts/build_openmath_original_clean_raw_dataset.py
"""
...
RAW_OUTPUT_DIR = Path("artifacts/raw_datasets/openmath_original_clean")
CHAT_OUTPUT_DIR = Path("artifacts/rendered_datasets/openmath_original_clean_qwen3_disable_thinking")
SYSTEM_PROMPT = (
...
def main() -> None:
    ...
    groups_by_problem: dict[str, list[Row]] = defaultdict(list)
    for source in SOURCES:
        for row in kept_rows[source]:
            groups_by_problem[canonical_problem(row.problem)].append(row)

    groups = list(groups_by_problem.values())
    rng.shuffle(groups)
    total_accepted_rows = sum(len(rows) for rows in kept_rows.values())
    target_val_rows = round(total_accepted_rows * VAL_RATIO)
    val_groups: list[list[Row]] = []
    train_groups: list[list[Row]] = []
    val_row_total = 0
    for group in groups:
        if val_row_total < target_val_rows:
            val_groups.append(group)
            val_row_total += len(group)
        else:
            train_groups.append(group)

    train_rows = [row for group in train_groups for row in group]
    val_rows = [row for group in val_groups for row in group]
    train_source_counts = Counter(row.source for row in train_rows)
    val_source_counts = Counter(row.source for row in val_rows)
    split_counts = {
        "train": {source: train_source_counts[source] for source in SOURCES},
        "val": {source: val_source_counts[source] for source in SOURCES},
    }
    split_group_counts = {
    ...
    raw_manifest = {
        ...
        "split_rule": "group rows by canonical problem text before train/validation split so repeated or answer-variant solutions for the same problem cannot cross splits",
